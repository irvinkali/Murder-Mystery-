...
- 2026-07-29 (Claude Code, cont.): **Local run + Step 6 (software half) done.** `engine/dev-server.js` runs the whole app locally with zero deps/zero install (`node engine/dev-server.js` → localhost:8888; in-memory state). `engine/smoke.js` boots it and tests the real HTTP wiring (static routing + /api functions + reveal gating): **10/10 PASS**. `npm run check` now runs build+validate+test(30/30)+smoke(10/10)+spoiler-scan(CLEAN) — the single pre-commit gate. Runbook + README updated with local-preview instructions.
  Remaining:
  - Step 4 extra — auto-fire *variant-specific* branching consequences (defense drops, alibi contradiction, medical-file reveal). The MECHANISM is straightforward but the drop CONTENT is new plot → same authoring blocker as flex (Step 1). Deferred pending authoring route.
  - Step 1 — flex characters F1–F10 (blocked on spoiler-safe authoring route).
  - Step 6 — live HARDWARE pass only (real Netlify deploy + Blobs, physical NFC tags, iPad/AirPlay). Software path is tested locally; hardware needs the physical setup / a deploy and can't be done from here.
